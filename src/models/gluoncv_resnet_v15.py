...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, dilated=True,dropout=0,
        ...
        self.conv1 = nn.Sequential(
            ...
            norm_layer(fm[0]),
            nn.ReLU(inplace=True),
            nn.Conv2d(fm[0], fm[1], kernel_size=3, stride=1, padding=1, bias=False),
        )
        '''
        self.conv1 = nn.Conv2d(4, fm[0], kernel_size=7, stride=2, padding=1, bias=False)
        ...
        for name,m in self.named_modules():
            if isinstance(m, nn.Conv2d):
                ...
                if ( 'layer' in  name ) and ( (  (block == BasicBlock)  and ('bn2' in name) )  or (  (block == Bottleneck ) and ('bn3' in name) )):
                    print(name , ' : zero gamma init' )

                    #zero gamma
                    m.weight.data.zero_()
                    m.bias.data.zero_()
                else:
                    print( name , ' : normal gamma init')
