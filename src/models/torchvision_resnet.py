...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, input_shape=(224,224) ,dropout = 0.5 ,   num_classes=1000):
        self.inplanes = 64
...
def warp_dict_fn(d):
    ...
    conv1_weight = torch.cat( [conv1_weight ,  conv1_weight[:,0].reshape( 64,1,7,7 ).clone()  ]  , dim = 1 )
...
def resnet34(pretrained=False, **kwargs):
    ...
    if pretrained:
        d = model_zoo.load_url(model_urls['resnet34'])
        d = warp_dict_fn( d )
        try:
